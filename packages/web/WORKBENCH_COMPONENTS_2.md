### Workbench Architecture v1.4 – Components and Data Flow

## Overview

This document describes the v1.4 workbench after the web focus cleanup:
...
packages/web/
  src/components/workbench/
    ...
    blocks/                   # Extracted from tool-blocks.tsx
    ...
    tool-blocks.tsx           # Re-export façade (backward compat)
    tool-call-block.tsx       # Generic expandable container
    a2a-card.tsx              # A2A Agent Card display component
    discovery-success-block.tsx # Success summary
    tool-list-summary.tsx     # Capabilities summary
  ...
  src/hooks/
    ...
    chat-engine/              # Extracted from use-chat-engine.ts
      index.ts                # Barrel exports
      types.ts                # ChatLogMessage, EngineState, EngineCommand, Action
      reducer.ts              # Reducer + initialState + helpers (uniqueId, isValidDomain)
      orchestration.ts        # processDomain + provideAuth (stateless functions)
...
## Module Decomposition (v1.4 changes)

### tool-blocks.tsx → blocks/

| Before (557 lines)                                                       | After                                            |
| ------------------------------------------------------------------------ | ------------------------------------------------ |
| `DiscoveryToolBlock` + `DiscoveryDetailsView`                            | `blocks/discovery-block.tsx`                     |
| `ConnectionToolBlock` + `ProtocolGuidanceView` + `ConnectionDetailsView` | `blocks/connection-block.tsx`                    |
| Shared types + `getStepStatusClassName`                                  | `blocks/shared.tsx` (+ `StepTimeline` component) |
| `ToolBlocks` summary component                                           | `tool-blocks.tsx` (re-export façade)             |
...
| processDomain + provideAuth        | `chat-engine/orchestration.ts` |
